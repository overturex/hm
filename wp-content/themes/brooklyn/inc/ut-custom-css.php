...
if ( !function_exists( 'unitedthemes_custom_css' ) ) {
	...
	function unitedthemes_custom_css() {
        ...
        $css  = '<style type="text/css">'. "\n";
			...
			$css .= 'p.lead span, .entry-title span, #cancel-comment-reply-link, .member-description-style-3 .ut-member-title,  .ut-twitter-rotator h3 a, .themecolor  { color: ' . $accentcolor . '; }'. "\n";			
			...
			if( ( ( is_front_page() || is_page() ) && ot_get_option('ut_front_video_state') == 'on' ) || ( ( is_home() || is_single() ) && ot_get_option('ut_blog_video_state') == 'on' ) ) :
				...
				if( $detect->isMobile() || $detect->isTablet() ) :
					...
					$css .= '.ut-video-poster {
						...
						overflow: hidden;
						opacity: 1;
						background:url("'.$ut_video_poster.'");
					...
					if ( $css_query->have_posts() ) :
						...
						while ( $css_query->have_posts() ) : $css_query->the_post();
							 ...
							 */
								...
								$css .= '.parallax-header, .section-header { padding-bottom:' . $ut_section_slogan_padding . '; }'. "\n";									
