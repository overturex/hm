		...
		if ( 0 < count( $field_data ) ) {
			...
			foreach ( $field_data as $k => $v ) {
				...
				if( $v['type'] == 'taxonomy' ) {
                    ...
                    /* image cropping */
					$html .= '<tr valign="top">';
                        $html .= '<th scope="row">' . __('Activate Image Cropping' , 'ut_portfolio_lang') . '</th>';
                        $html .= '<td><div class="ut-checkbox"><input name="' . esc_attr( $k ) . '[image_cropping]" type="checkbox" id="' . esc_attr( $k ) . '_image_cropping" ' . $this->checked_array( 'image_cropping' , $data ) . ' /> <label for="' . esc_attr( $k ) . '_image_cropping"></label></div>';
                        $html .= '<p class="description">' . __('only valid for this showcase.' , 'ut_portfolio_lang') . '</p>';
                        
                        /* optional setting , is hidden when copping is disabled */
